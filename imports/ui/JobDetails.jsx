...
export default class JobDetails extends React.Component {
	...
	handlePostReply(e) {
		...
		Meteor.call('job.postReply', this.state.job.name, e.target.id, this.state.replyField, _err => {
			this.setState({
				replyField: '',
				replyFieldId: -1
			});

			Meteor.call('job.getFromName', this.state.job.name, (_error, job) => {
				this.setState({
					job
				});
			});
			...
			jobComments.push(
				<div key={i} style={{ paddingBottom: '15px' }}>
					<ListGroupItem>
						...
						<a href={`/profile/${this.state.job.comments[i].username}`}>
							{' '}
							-
							{this.state.job.comments[i].username}
						</a>
					</ListGroupItem>
					{this.state.job.comments[i].replies.map(x => (
						<ListGroupItem key={x.text} style={{ paddingLeft: '30px' }}>
							<span style={{ fontWeight: 'bold' }}>Reply from OP: </span>
							{' '}
							{x.text}
						</ListGroupItem>
					))}
