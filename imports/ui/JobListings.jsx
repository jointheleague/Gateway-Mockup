            ...
            </div>
            {
              ...
              if(shouldReturn){
                const langLabels = job.langs.map((lang) => {
                  return(<span style={{"padding-right": "5px"}} key={job.toString() + lang.toString()}><Label bsStyle="default">{lang}</Label></span>);
                });
                return (
                  <div key={job.name + job.desc + job.client + job.level + Math.random()}>
                    <Panel>
                      <h3> {job.name} </h3>
                      <div className="currentTextDisabledSmall"> Posted By <a href="#"> {job.client} </a> </div>
                      <p> {job.desc} </p>
                      <br/>
                      <span style={{"padding-right": "5px"}}><Label bsStyle="primary">Lv. {job.level}</Label></span>
